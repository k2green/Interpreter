...
namespace InterpreterLib.Diagnostics {
	internal class BoundTreeDisplayVisitor : BoundTreeVisitor<IEnumerable<string>, string, string> {
		...
		protected override IEnumerable<string> VisitExpression(BoundExpressionStatement statement, string prefix1, string prefix2) {
			IEnumerable<string> line = new string[] { $"{prefix1}ExpressionStatement" };

			return line.Concat(Visit(statement.Expression, prefix2 + LAST_CHILD, prefix2 + SPACING));
		}
		...
		protected override IEnumerable<string> VisitForStatement(BoundForStatement statement, string prefix1, string prefix2) {
			...
			line = line.Concat(Visit(statement.Assignment, prefix2 + NEXT_CHILD, prefix2 + NO_CHILD));
			return line.Concat(Visit(statement.While, prefix2 + LAST_CHILD, prefix2 + SPACING));
		}
